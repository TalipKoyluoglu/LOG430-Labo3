...
class UC3_PerformanceControleur:
    ...
    def get_indicateurs_par_magasin(self):
        ...
        for magasin in magasins:
            ...
            chiffre_affaires = LigneVente.objects.filter(
                vente__magasin=magasin
            ).annotate(
                total_ligne=F("quantite") * F("prix_unitaire")
            ).aggregate(
            ...
            ventes_recente = Vente.objects.filter(magasin=magasin, date_vente__gte=semaine_dernier_jour)
            lignes_recent = LigneVente.objects.filter(vente__in=ventes_recente).values(
                "produit__nom"
            ).annotate(
                total_vendu=Sum("quantite")
            ).order_by("-total_vendu")[:3]

            tendances = [f"{ligne['produit__nom']} ({ligne['total_vendu']})" for ligne in lignes_recent]
